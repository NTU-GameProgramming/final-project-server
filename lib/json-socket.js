...
JsonSocket.prototype = {
	CODE: {
		...
		//console.log('[JSocket] onData(' +this._socket.remoteAddress + '/'+ this._socket.remotePort + ')' + data.toString());
		//console.log("How many listeners?"+this._socket.listeners('message').length);
...
//		console.log('parsed' + JSON.stringify(message));
		//console.log('[JSocket] _handleMessage(' +this._socket.remoteAddress + '/'+ this._socket.remotePort + ')' + data.toString());
...
// register
delegates = ['on', 'removeListener', 'connect', 'end', 'address'];
